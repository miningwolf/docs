...
Called when a block spreads based on world conditions. <p> Use [`BlockFormEvent`](BlockFormEvent.md) to catch blocks that "randomly" form instead of actually spread. <p> Examples: <ul> <li>Mushrooms spreading. <li>Fire spreading. </ul> <p> If a Block Spread event is cancelled, the block will not spread.
