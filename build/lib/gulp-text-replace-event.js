...
var PLUGIN_NAME = "gulp-text-replace-event";
   ...
   stream._transform = function(file, unused, done) {
    ...
    if (file.isBuffer()) {
           ...
           if (mainItem.namespace && mainItem.namespace.substr(0,17) === 'wolfscript.event.')
             {
               ...
               var newName = mainItem.name.substr(0, mainItem.name.length-5);


    
                mainItem.title = mainItem.title
                .replace(new RegExp(oldName, 'g'), newName + ' Event')
                .replace("hook", "event");
      ...
      text = JSON.stringify(result);
        ...
        var changes = [];
        text = text.replace(/(wolfscript\.event\.[0-9a-zA-Z]+\.([0-9a-zA-Z]+))Event/g, 
          function(x, p1, p2){changes.push(p2); return p1;});

       if (changes.length>0)
          changes.forEach(function(change){
           text = text.replace( new RegExp(change + 'Event', 'g'), function(x){return change;});        
          });
